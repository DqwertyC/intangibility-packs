...
execute as @e[tag=phase_marker] at @s unless block ~ ~ ~ minecraft:air unless block ~ ~ ~ minecraft:command_block run function phasing:display_to_block_b

# Start solidifying blocks
execute as @e[tag=phase_marker] at @s unless entity @a[predicate=phasing:can_phase,distance=0..3] run function phasing:display_to_block_a